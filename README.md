...
1. 增加“快速添加任务”功能，点击按钮后弹出添加界面，左侧选择其父节点（自上而下选择路径）、右侧编辑任务名称和属性，点确定即可在选择的父节点下新建此子任务，从而添加任务时可以省去拖动寻找位置的麻烦（其实就类比一下如何在文件管理系统中新建文件就行）

2. 同样的逻辑，可以在日程板的特定时间板块中“快速选择任务”

3. 优化：日程板中也可以点击完成任务了

4. 优化：在日程板中点击一个任务后，跳转任务的同时不会再关闭日程板

5. 修复：在主视图以外视图中点击某任务后，可以更准确地将其定位在视图中心

6. 优化：日程板中的任务可以通过点击其最左侧的箭头改变排列顺序

7. 修复：解决了日程板中任务过多时显示不下的问题（现在可以上下滚动）

8. 新增左下角的“关于与更新”按钮，之后更新不会弹窗提醒，而是会在这个按钮右上角红点提醒。点击这个按钮后会显示当前版本以及新版本（如有）的更新日志，并且可一键跳转Github相应界面
